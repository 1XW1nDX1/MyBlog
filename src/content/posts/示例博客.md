...
2. 在文件中添加`frontmatter`（前置元数据），这是文章的配置信息，必须包含`title`和`description`字段。  
...
2. 在新创建的文件夹中创建一个名为index.md的文件。

3. 在`index.md`文件中添加`frontmatter`（前置元数据），这是文章的配置信息，必须包含`title`和`description`字段：
